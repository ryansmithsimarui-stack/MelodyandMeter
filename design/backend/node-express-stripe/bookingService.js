...
function isResourceIdAllowed(resource_id){
  // Dynamic catalog overrides env allowlist if present
  try{
    const catalog = typeof persistence.listResources === 'function' ? persistence.listResources() : [];
    const activeIds = catalog.filter(r=>r.active).map(r=>r.id);
    if(catalog.length){ // enforce even if no active resources (all inactive => none allowed)
      return activeIds.includes(resource_id);
    }
...
function createBookingAndTrack({ booking_id, user_id, slot_id, start_at, duration_min, resource_id }) {
  ...
  // Record utilization snapshot after booking creation to feed history/forecast
  try{ if(typeof persistence.recordResourceUtilizationSnapshot === 'function'){ persistence.recordResourceUtilizationSnapshot('booking'); } }catch(e){ /* ignore snapshot errors */ }
  return { booking };
}
